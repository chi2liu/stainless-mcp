...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['FIRST_STAINLESS_MCP_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['FIRST_STAINLESS_MCP_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the First Stainless Mcp API.
 */
export class FirstStainlessMcp {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['FIRST_STAINLESS_MCP_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('FIRST_STAINLESS_MCP_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.FirstStainlessMcpError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the FirstStainlessMcp client with an apiKey option, like new FirstStainlessMcp({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? FirstStainlessMcp.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('FIRST_STAINLESS_MCP_LOG'), "process.env['FIRST_STAINLESS_MCP_LOG']", this) ??
      defaultLogLevel;
  ...
  static FirstStainlessMcp = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static FirstStainlessMcpError = Errors.FirstStainlessMcpError;
...
FirstStainlessMcp.PetResource = PetResource;
FirstStainlessMcp.Store = Store;
FirstStainlessMcp.UserResource = UserResource;
export declare namespace FirstStainlessMcp {
