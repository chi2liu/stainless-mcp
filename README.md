...
```sh
npm install git+ssh://git@github.com:stainless-sdks/first-stainless-mcp-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install first-stainless-mcp`
...
const response: FirstStainlessMcp.StoreListInventoryResponse = await client.store.listInventory();
```
...
const response = await client.store.listInventory().catch(async (err) => {
  if (err instanceof FirstStainlessMcp.APIError) {
...
1. Via the `FIRST_STAINLESS_MCP_LOG` environment variable
...
const client = new FirstStainlessMcp({
  logger: logger.child({ name: 'FirstStainlessMcp' }),
...
const client = new FirstStainlessMcp({ fetch });
```
...
```ts
import FirstStainlessMcp from 'npm:first-stainless-mcp';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/first-stainless-mcp-typescript/issues) with questions, bugs, or suggestions.
